 ...
 * game.h
 * 
 * Header file consisting attributes and methods that a board game would have. 
...
namespace main_savitch_14 {
    class Game {
        public:
            ...
            // The play method should not be overridden. It plays one game,
        ...
        protected:
            ...
            // Virtual method to get the user move object
            virtual std::string get_user_move() const;

            // Virtual method to determine which player is the previous mover
            virtual who last_mover() const {return (move_num % 2 == 1 ? HUMAN : COMPUTER);}

            // Virtual method to compute the value of moves completed
            ...
            // Virtual method to determine who wins the round of the game
            virtual who winning() const;

            // *******************************************************************
            // VIRTUAL METHODS THAT MUST BE OVERRIDDEN (Overriding method
            // should call the original when it finishes)
            // *******************************************************************

            // Virtual method to have the next player make a specified move
            virtual void make_move(const std::string& move) {++move_num;}

            // Virtual method to restart the game from the beginning
            virtual void restart() {move_num = 0;}
           ...
           // Pure virtual method to display the status of the game
            virtual void display_status() const = 0;

            // Pure virtual method to evaluate a board position
            virtual int evaluate() const = 0;

            // Pure virtual method to determine whether or not the game is finished
            virtual bool is_game_over() const = 0;

            // Pure virtual method to determine whether or not the given move is legal for the next player
        ...
        private:
            ...
            // Private method that evalauates with look ahead
            int eval_with_lookahead(int look_ahead, int beat_this);

            // Private method to make a computer move
            void make_computer_move();

            // Private method to make a human move
